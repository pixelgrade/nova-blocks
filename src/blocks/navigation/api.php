<?php
/**
 * This code has been taken from the following plugin:
 * @link https://wordpress.org/plugins/wp-rest-api-v2-menus/
 *
...
if ( ! function_exists( 'wp_api_v2_menu_get_all_locations' ) ) {
	/**
	 * Get all locations
	 * @return stdClass List of locations
	 **/
...
if ( ! function_exists( 'wp_api_v2_locations_get_menu_data' ) ) {
	/**
	...
	function wp_api_v2_locations_get_menu_data( $data ) {
		...
		if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $data['id'] ] ) ) {
			...
			return new WP_Error( 'not_found', sprintf( esc_html__('No menu has been found with this id or slug: `%s`. Please ensure you passed an existing menu ID, menu slug, location ID or location slug.', '__plugin_txtd' ), esc_html( $data['id'] ) ), array( 'status' => 404 ) );
		}
...
if ( ! function_exists( 'wp_api_v2_menus_dna_test' ) ) {
	/**
...
if ( ! function_exists( 'wp_api_v2_menus_get_menu_items' ) ) {
	/**
...
if ( ! function_exists( 'wp_api_v2_menus_get_menu_data' ) ) {
	/**
	 * Get menu's data from his id.
