...
function novablocks_agent_blocks_ability_definitions(): array {
	return [
		...
		'pixelgrade/describe-block'     => [
			...
			'output_schema'       => novablocks_agent_blocks_output_schema(
				[
					...
					'save_body'                => [
						'type'        => 'string',
						'description' => 'Whether authored block comments need a static saved body or are dynamically rendered.',
					],
					'body_template'             => [
						'type'        => [ 'string', 'null' ],
						'description' => 'Harness-generated canonical inner-HTML skeleton for a parameterized static block; null when no complete fillable template is curated.',
					],
					'body_template_slots'       => [
						'type'        => 'array',
						'items'       => [ 'type' => 'string' ],
						'description' => 'Attribute names whose placeholders must all be replaced in body_template.',
					],
					'body_template_constraints' => [
						'type'                 => 'object',
						'additionalProperties' => true,
						'description'          => 'Save-affecting attributes that must equal these values for body_template to remain canonical.',
					],
					'body_template_note'        => [ 'type' => 'string' ],
					'coverage'                  => [
						'type'                 => 'object',
						'additionalProperties' => true,
					],
		...
		'pixelgrade/validate-post'      => [
			...
			'description'         => __( 'Check whether the blocks stored in one or more posts parse the way the block editor would parse them. This is the fact that settles "does this markup actually load in the editor?" — rendering correctly on the front end does not prove it, and content written through the REST API or WP-CLI routinely parses invalid while looking fine. Give it "post_ids"; "post_type" asserts every id is of that type (a mismatch is invalid_params, not a silent skip), and "all_parts": true also walks the site\'s database-resident wp_template and wp_template_part posts, where the recurring failure is a missed footer part. Writes nothing. Findings come back as a successful call with code "invalid_blocks" and data.invalid[] entries of {post_id, index, block_name, reason_code} — block identity only, never any of the stored content. It ALSO applies a fixed-point post-condition: after per-block validity it checks that serialize(parse(content)) equals the stored markup, because a block that parses valid only against a DEPRECATED save reports isValid:true and is silently rewritten the next time the editor saves. A document that fails it comes back with code "not_canonical" (or, alongside genuine invalid blocks, as a "not_canonical" warning) and data.not_canonical[] entries of {post_id, index, block_name, reason_code}, where reason_code is "valid_via_deprecation" for a named block or "not_a_fixed_point" for a document-level byte difference. Treat it as blocking: this is the check that distinguishes markup that loads from markup that survives a save. PRECONDITION: this needs the separately-installed Pixelgrade agent-tools harness, a Node package that ships apart from the plugin; without it the call fails with code "harness_unavailable" and a summary naming the install step. Requires edit_posts, plus edit_post on every requested id.', '__plugin_txtd' ),
			'annotations'         => [
				'readonly'    => true,
				'destructive' => false,
				'idempotent'  => true,
			...
			'output_schema'       => novablocks_agent_blocks_output_schema(
				[
					...
					'not_canonical' => novablocks_agent_blocks_object_list_schema(),
					'harness'       => [
						'type'                 => 'object',
						'additionalProperties' => true,
					],
