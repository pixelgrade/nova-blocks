...
**A. Server registration** — `register_block_type_args` merges six attributes into `core/group`
and `core/columns`:
...
The merge **appends** and never reorders. That is a byte-identity constraint, not a style
preference: the server registration is what `get_block_editor_server_block_settings()` hands the
agent harness, and `serialize()` walks `blockType.attributes` in order to build the block
comment's JSON. Prepending would reorder the keys of every already-stored Group.
...
- it emits **only** attributes explicitly present in the stored attrs AND differing from their
  ...
  untouched;
- it **stands down entirely** when the rendered markup already contains
  `--nb-block-top-spacing` — an editor save wrote the whole bundle and that markup is
  authoritative; never doubled, never overridden by a stale comment attribute;
...
- `tests/php/core-container-spacing-contract.php` — attribute slice, append-not-prepend ordering,
  scope (core/column and everything else untouched), the no-op guarantee, the emit, formatting.
