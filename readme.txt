...
* Improved: Keep full-width Search facets usable on compact screens and prevent plugin search minimums from overlapping mobile filter controls.
...
* Improved: Preserve user-authored CSS classes on header navigation items through the block-editor navigation round-trip, so custom classes (for example call-to-action or external-link styles) survive projecting to and from the menu system.
...
* New: Add the below-content metadata position for post cards.
* Fix: Restore multi-column masonry packing on the frontend and in the editor preview.
* Fix: Refactor contextual post cards to use the shared runtime palette tokens.
