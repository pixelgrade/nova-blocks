...
- Applying a preset is ONE `setAttributes()` patch: write every declared value, clear every managed attribute the preset omits BY WRITING ITS REGISTERED DEFAULT (pass `registeredDefaults` to `getPresetApplyPatch`; a literal `undefined` is the fallback only when no default exists), preserve everything outside the managed set. No follow-up attribute writes (one-step undo is guaranteed by this shape). Never clear with a literal `undefined` when a registered default exists: the live editor keeps `undefined` as-is until save + reparse (blank previews — the 2026-08-29 Cards Collection composition-switch bug), while writing the default is serialization-neutral (WP omits default-equal attributes) and derivation-neutral (`deriveActivePresetId` normalizes through the same defaults).
...
- A tile must store exactly what the editor settles on at mount (`getUpdatedAttributes()` via `withUpdatedAttributes`), or reloads turn the post dirty and the tile derives as Custom (`color-tiles-editor-mount.test.js`). Button declares `stickySourceColor: 'keep'`: the mount keeps an existing own-palette source reference but nothing snaps into one, and an inactive Button's first Color Signal change updates as `false` (its latent `useSourceColorAsReference: true` default never becomes a live reference). Action v2 therefore stores the REFERENCE and follows the brand color through palette changes (a kept reference mirrors its stored variation 1 into `contentPaletteVariation`, so only `colorSignal`, the source's signal against its surface, can move when the palette changes); a v1 Action button (explicit step) keeps its markup and derives as Custom. Both the Presets tab (`getColorTileMountPatch`) and the writer (`novablocks_color_tiles_mount_normalize`) fold the mount's `contentPaletteVariation` into the one patch.
...
- Both container lists are generated from `packages/utils/src/layout-containers.js` (single source; SCSS twin is generated + test-pinned): `LAYOUT_GRID_CONTAINERS` → `$nb-layout-grid-parents` (the root union) and `LAYOUT_PASSTHROUGH_CONTAINERS` → `$nb-layout-passthrough-containers` (the track-neutral subset). Edit only there.
- A plain reading-flow Group is a subgrid PASS-THROUGH but never a root, so it is deliberately absent from the root union — which means the engine's `:is($nb-layout-grid-parents) > &` subgrid override can never reach a Group's own children. `blocks/core/group/style.scss` therefore repeats that override for direct `$nb-layout-passthrough-containers` children of a qualified Group, behind the same `above(lap)` + `@supports (grid-template-columns: subgrid)` gates. Both override sites must consume the generated list (test-pinned); no id-level specificity anchor is needed there.
...
- A Group with core's "Inner blocks use content width" OFF (explicit `layout.type: default`; render filter + editor twin add `nb-group--fill`) lets its default-aligned children follow its width: no content-width cap, and inside the pass-through a Wide/Full fill Group puts them on its own wide/full track (GitHub #657). Only a fill Group placed directly in a layout container (`$nb-layout-grid-parents`, minus the `.wp-site-blocks` root) lifts the cap; anywhere else, e.g. a canvas Group in `.wp-site-blocks`, its children keep the cap (GitHub #671). A Group without a layout attribute keeps the cap (byte- and pixel-identical).
- The lifted (0,4,0) grid-child `width: 100%` stretch (#660) never reaches the header: a `header` template part and the Nova header chrome (`$nb-layout-header-chrome`) keep the (0,2,0) stretch, so theme pins such as Anima's Border Site Frame header win (GitHub #670).
...
- Divider rule (#658, on the #668 rule model): `ruleRole` (`primary`/`secondary`, no default) plus an optional `ruleWeight` override (1–4px). Off serializes and renders nothing, and both attributes stay out of the data-* attributes and the layout recipes. The line is the Sidecar's `::before`, a grid item on row 1 over `gs / ge` inside `nb-sidecar-desktop-layout` (`order: -2`, one background layer per side, drawn only while that rail exists and has content). `init.php` and `rule-style.js` are pinned to `rule-cases.json`. The role tokens `--nb-rule-{primary,secondary}-{weight,color}` live in core `_variables.scss`; `--nb-rule-weight`, `--nb-rule-color` and `--nb-rule-strong-color` are their aliases.
