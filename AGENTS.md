...
- Styles remain `is-style-simple` (lines), `is-style-decorative` (symbol), and `is-style-blank` (spacer).
- Line thickness is the `ruleWeight` attribute (shared `RuleControls`, 1–4px, weight only; colour stays with Color Signal), offered on lined styles only (`is-style-simple`, Anima's `is-style-elaborate`). The registered default `3` emits and serializes nothing; other values emit `--nb-separator-rule-weight` from `init.php` and `rule-style.js`, both pinned to `rule-weight-cases.json`. Anima declares `--separator-line-thickness: var(--nb-separator-rule-weight, 3px)` on `.c-separator`, never on `:root`, so it resolves per block (pixelgrade/anima#610). Historical Anima override details belong in the relevant issue or `.ai/` investigation, not this runbook.
