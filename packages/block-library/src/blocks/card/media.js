...
const CardMedia = ( props ) => {
  ...
  if ( !! mediaURL ) {
    return (
      <Fragment>
        { media.type !== 'video' &&
          <img className={ `nb-supernova-item__media` } src={ mediaURL } alt={ media?.alt } /> }
        { media.type === 'video' &&
          <video muted autoPlay loop playsInline className={ `nb-supernova-item__media` } src={ media?.url } /> }
      </Fragment>
    )
  }
