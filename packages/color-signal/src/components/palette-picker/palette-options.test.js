...
test( 'getVisiblePalettes returns brand palettes by default, in payload order', () => {
  expect( getVisiblePalettes( PALETTES ).map( p => p.id ) ).toEqual( [ '1', '2', '3' ] );
} );

test( 'getVisiblePalettes returns functional (underscore-id) palettes when toggled', () => {
  expect( getVisiblePalettes( PALETTES, true ).map( p => p.id ) ).toEqual( [ '_info', '_warning' ] );
} );

test( 'getVisiblePalettes tolerates a missing/non-array payload', () => {
  expect( getVisiblePalettes( undefined ) ).toEqual( [] );
  expect( getVisiblePalettes( null, true ) ).toEqual( [] );
} );

test( 'isFunctionalPaletteOption matches the underscore-prefix rule of @novablocks/utils isFunctionalPalette', () => {
...
test( 'getNextPalette starts from the first entry when the current palette is not in the list', () => {
  const brand = getVisiblePalettes( PALETTES );
...
test( 'getNextPalette compares ids loosely across string/number types', () => {
...
test( 'getPaletteLabel falls back to "Palette N" for unlabeled palettes', () => {
...
test( 'getPaletteDisplayColor prefers the sidebar swatch color: source[0]', () => {
