...
const handleClassicGrid = ( $grid, $block, attributes ) => {
...
const handleMasonryGrid = ( $grid, $block, attributes ) => {
  const $children = $grid.children();
  const { columns } = attributes;

  const groups = $children.toArray().reduce( ( acc, curr, index ) => {
  ...
  groups.forEach( group => {
    const $column = $( '<div class="nb-collection__layout-column">' );
...
const handleParametricGrid = ( $grid, $block, attributes ) => {
