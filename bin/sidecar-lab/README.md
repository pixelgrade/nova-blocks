...
- **54 published pages**, slug prefix `sidecar-lab-` (plus one `sidecar-lab-tpl-*` wp_template per template fixture, swept by the same prefix). The first 17 are the
  ...
  **baseline-v2b** re-baseline.
  ...
  | **Aligned images in the reading column (k) — GitHub #656** | |
  | `template-single-aligned-right`, `template-single-aligned-left`, `template-single-aligned-railless`, `aligned-page` | the (j) single-style templates (right / left rail, rail-less) and a plain page, whose Post Content holds an alignleft 190x240 and an alignright 300px image with text beside them. Capture them twice: Content Inset UNSET (the compat pin) and SAVED (`sm_content_inset` 230, run label suffix `-inset`): with the inset, a broken pull-out sits on its half of the reading column (cs-gcs / gce-ce) at its authored width with the next block beside it, and one too wide for that half stays unbroken in cs-ce |
  | **Wide post header beside a rail (l) — GitHub #655, revised decision 3** | |
  | `template-single-wide-right`, `template-single-wide-left`, `template-single-wide-both`, `template-single-wide-railless` | the (j) single-style templates with the post-title, a meta row (flex Group, class `sl-post-meta`: author + date) and the featured image set to Wide; Post Content default (reduced battery). Capture UNSET (compat pin) and SAVED (`sm_content_inset` 230): with the inset, wide beside a rail ends at the rail gap (`acs` / `ace`, the pre-inset content edge — the same x as the unset `cs` / `ce`), the free side still reaches `ws` / `we`, and the text keeps the inset reading column `cs`-`ce` |
